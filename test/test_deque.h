...
TEST(deque, givenSomeEmptyNodes_whenShrinkToFit_thenNoMoreEmptyNodes) {
    static int dtor_counter = 0;
    static int ctor_counter = 0;
    static int cpyctor_counter = 0;
    struct test_struct {
    ...
    auto sut = stl::deque<test_struct, 3>{};
    sut.emplace_back(1);
    sut.emplace_back(2);
    sut.emplace_back(3);
    sut.emplace_back(4); // Trigger a back-extension
    EXPECT_EQ(4, sut.size());
    EXPECT_EQ(2, sut.map_size);
    sut.pop_back();
    EXPECT_EQ(2, sut.map_size);
    EXPECT_EQ(1, dtor_counter);
    sut.shrink_to_fit();
    EXPECT_EQ(1, sut.map_size);
    auto it = sut.begin();
    EXPECT_EQ(1, (it++)->v);
    EXPECT_EQ(2, (it++)->v);
    EXPECT_EQ(3, (it++)->v);
...
TEST(deque, givenSomeEmptyNodesFront_whenShrinkToFit_thenNoMoreEmptyNodes) {
    static int dtor_counter = 0;
    static int ctor_counter = 0;
    static int cpyctor_counter = 0;
    struct test_struct {
    ...
    auto sut = stl::deque<test_struct, 3>{};
    sut.emplace_front(1); // Trigger a front-resize
    EXPECT_EQ(1, sut.size());
    EXPECT_EQ(2, sut.map_size);
    sut.pop_back(); // Empty initial node
    EXPECT_EQ(2, sut.map_size);
    EXPECT_EQ(1, dtor_counter);
    sut.shrink_to_fit();
    EXPECT_EQ(0, sut.size());
    EXPECT_EQ(0, sut.map_size);
}

TEST(deque, givenEmpty_whenShrinkToFit_thenNoNodes) {
    static int dtor_counter = 0;
    static int ctor_counter = 0;
    static int cpyctor_counter = 0;
    struct test_struct {
